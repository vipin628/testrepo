...
rating.selectedIcon = "assets/images/icons/selectedStar.svg";
rating.unselectedIcon = "assets/images/icons/unselectedStar.svg";
...
rating.create = function (settings) {
  ...
  var subSelector = "img";
  if (
    this.selectedIcon.substring(0, 3) == "fa " ||
    this.unselectedIcon.substring(0, 3) == "fa "
  ) {
    usingFa = true;
    subSelector = "i";
  ...
  $(settings.selector + " " + subSelector).on("mouseout", function () {
    var selected = $(this)
      .siblings("input[name=" + rating.name + "]")
      .val();
    $(settings.selector + " " + subSelector).each(function (i, e) {
      if (i < selected) {
        $(e).attr("src", rating.selectedIcon);
